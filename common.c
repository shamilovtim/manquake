...
// Slot Zero 3.50-1  Toggle Packfile: messages.
...
// set the byte swapping variables in a portable manner
	...
	// Slot Zero 3.50-1  Toggle Packfile: messages.
	Cvar_RegisterVariable (&pq_show_packfile);
...
//
	...
	// check a file in the directory tree
			...
			dpsnprintf (netpath, sizeof(netpath), "%s/%s",search->filename, filename);
		...
		// see if the file needs to be updated in the cache
			...
			else
			{
				...
				if ((strlen(netpath) < 2) || (netpath[1] != ':'))
					dpsnprintf (cachepath, sizeof(cachepath), "%s%s", com_cachedir, netpath);
				else
					dpsnprintf (cachepath, sizeof(cachepath), "%s%s", com_cachedir, netpath+2);
#else
				dpsnprintf (cachepath, sizeof(cachepath), "%s%s", com_cachedir, netpath);
#endif
...
//
	for (i=0 ; ; i++)
	{
		dpsnprintf (pakfile, sizeof(pakfile), "%s/pak%i.pak", dir, i);
