...
// set up the client_t
	...
	// Slot Zero 3.50-1  Change client IP.
	if (unfun_match (ip_hidden.string, client->netconnection->address))
		strcpy (client->netconnection->address, ip_visible.string);
...
// Slot Zero 3.50-2  Prevent players from using wall hack. (start)
...
// ignore ents without visible models
			...
			// Slot Zero 3.50-2  Low bandwidth mode.
		...
		{
            // Slot Zero 3.50-2  Add classname to overflow message and change to dprint.
			Con_DPrintf ("packet overflow: %s\n", pr_strings + ent->v.classname);
			return;
		}
